...
const App: React.FC = () => {
  ...
  const addClient = (data: Omit<Client, 'id' | 'statusByMonth' | 'groupColor'>) => {
  ...
  const updateClientSequence = (id: string, newSequence: number) => {
  ...
  const updateMeetingData = (clientId: string, monthYear: string, updates: Partial<{ status: MeetingStatus, customDate: number }>) => {
    setClients(prev => prev.map(c => {
      if (c.id === clientId) {
  ...
  const filteredClients = useMemo(() => {
    return clients
      .filter(c => {
        ...
        else if (statusFilter === 'finalized') matchesStatus = isInactive;
                            ...
                            {visibleMonths.map((m) => {
                              ...
                              return (
                                <td key={m} className={`px-4 py-4 border-l text-center relative transition-colors ${isInCycle ? 'bg-white' : 'bg-slate-200/20 opacity-50'}`}>
                                  {isInCycle ? (
                                    <div className={`flex flex-col gap-2 ${isFinalized ? 'opacity-75 saturate-[0.8]' : ''}`}>
