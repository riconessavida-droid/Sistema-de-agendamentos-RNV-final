...
const App: React.FC = () => {
  ...
  const checkIfNeedsAttention = (client: Client) => {
    ...
    for (const mKey of cycle) {
      ...
      // Se a reunião não foi feita nem o contrato encerrado
  ...
  const reportData = useMemo(() => {
    ...
    clients.forEach(c => Object.entries(c.statusByMonth).forEach(([m, s]: any) => {
      if (s.status === MeetingStatus.CLOSED_CONTRACT) map[m] = (map[m] || 0) + 1;
    }));
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <nav className="bg-white border-b sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={exportClientsToCSV} className="p-2 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors" title="Exportar CSV"><Download className="w-5 h-5 text-slate-600" /></button>
            <button onClick={() => { setEditingClient(null); setIsFormOpen(true); }} className="bg-yellow-500 text-white px-4 py-2 rounded-lg font-bold hover:bg-yellow-600 transition-all flex items-center gap-2"><Plus className="w-5 h-5" /> Novo Cliente</button>
            <button onClick={handleLogout} className="p-2 text-slate-400 hover:text-red-600 transition-colors"><LogOut className="w-5 h-5" /></button>
          </div>
        </div>
      </nav>
      ...
      <div className="bg-white border-b">
        <div className="max-w-[1600px] mx-auto px-4 flex gap-8">
          ...
          <button onClick={() => setActiveTab('checklist')} className={`py-4 text-xs font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'checklist' ? 'border-yellow-500 text-yellow-600' : 'border-transparent text-slate-400'}`}>Checklist</button>
        ...
        {activeTab === 'overview' ? (
                          ...
                          <td className={`px-4 py-4 sticky left-0 z-20 w-80 border-r shadow-sm transition-colors ${attention ? 'bg-orange-500 text-white' : inactive ? 'bg-slate-200 text-slate-500' : client.groupColor}`}>
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex flex-col gap-1">
                                <button onClick={() => { setEditingClient(client); setIsFormOpen(true); }} className="p-1 hover:scale-110 transition-transform"><Pencil className="w-4 h-4" /></button>
          ...
          <div className="bg-white p-8 rounded-3xl border shadow-sm animate-in fade-in">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h2 className="text-2xl font-black text-slate-800 flex items-center gap-3"><Trophy className="text-yellow-500 w-8 h-8" /> Conversão de Contratos</h2>
                <p className="text-sm text-slate-400 font-medium mt-1">Histórico de contratos encerrados por mês</p>
              </div>
            ...
            <div className="h-80 flex items-end gap-4 px-4 border-b pb-2">
              {reportData.map((d, i) => {
                const max = Math.max(...reportData.map(x => x.count), 1);
                const height = (d.count / max) * 100;
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-3 group">
                    <div className="relative w-full flex justify-center items-end h-full">
                      <div className="absolute -top-8 opacity-0 group-hover:opacity-100 transition-all bg-slate-800 text-white text-[10px] font-black px-2 py-1 rounded">{d.count}</div>
                      <div className="w-full max-w-[40px] bg-gradient-to-t from-yellow-500 to-yellow-300 rounded-t-lg transition-all duration-500 group-hover:from-yellow-600" style={{ height: `${height}%` }} />
                    </div>
                    <div className="text-center">
                      <p className="text-[9px] font-black text-slate-400 uppercase">{d.label.split(' ')[0]}</p>
                      <p className="text-[8px] font-bold text-slate-300">{d.label.split(' ')[1]}</p>
                    </div>
                  </div>
                );
              })}
